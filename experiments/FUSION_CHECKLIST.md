...
Last reconciled: 2026-09-21. Update this ledger with every implementation and result.
...
| D08 | cuBLAS QKV GEMM + packed RoPE/KV-write epilogue | **INCONCLUSIVE** | Its first H100 gate found exact Q/K but invalid V: V segments 14/15 were mapped to cache heads 2/3 instead of restarting at 0/1. The measured 2.08× B8 and 1.11× B64 numbers are invalid because the real V heads remained unwritten. The segment mapping is fixed but not yet rerun on H100. | `custom_kernels/packed_qkv_rope_cache.py`; `piecewise_fa3_packed_epilogue`; first failing result `fa3-fusion-kernel-gate-v2/` | Rerun the cheap gate. Proceed to the FA3 full-engine arm only if Q, K, and V all pass. |
...
| D10 | LM-head projection + exact greedy argmax without materializing `[B,V]` logits | **INCONCLUSIVE** | Already implemented and tested. Isolated graph-warm speedup was 1.055× at B8 and 1.186× at B64. In the complete 28-layer split-K graph it was only 1.009×/1.001×. Inside old K-step graphs it ranged from 0.995× to 1.009× versus the materialized head, with exact tokens and KV state. | `custom_kernels/fused_lm_head.py`; `output-head.json`; `output-head-full-model.json`; `unrolled-decode.json` | The existing kernel is now wired into the FA3 K=1/2/4/8 runner; execute that pending H100 run. Do not reimplement it. |
| D11 | K=1/2/4/8 whole-model graph experiment | **INCONCLUSIVE** (graph composition, not a kernel fusion) | The original split-K run retained every validation logit and passed logit/token/KV checks, but produced roughly 0.995–1.011×. The FA3 runner now retains all logits in a validation capture and compares materialized and fused-head production captures while reporting even sub-percent effects. | `engine/graph/unrolled_graph_decoder.py`; old `unrolled-decode.json`; `benchmark_decode_control_plane.py` | Execute the pending FA3 run, then add C++ chunk commit only if the correctness-passing effect is repeatable. |
