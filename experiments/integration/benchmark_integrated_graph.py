...
COMPARISONS = (("decode_graph", "eager"),
               ...
               ("piecewise_fa3_prefill_qkv_epilogue", "piecewise_fa3_prefill_control"),
               ("piecewise_fa3_prefill_residual_rmsnorm", "piecewise_fa3_prefill_control"),
               ("piecewise_fa3_prefill_both", "piecewise_fa3_prefill_control"),
               ("piecewise_fa3_prefill_both", "piecewise_fa3_prefill_qkv_epilogue"),
               ("piecewise_fa3_prefill_both", "piecewise_fa3_prefill_residual_rmsnorm"),
               ("piecewise", "eager"),
...
def build_parser():
    ...
    mode.add_argument("--prefill-fusions", action="store_true",
                      help="ablate packed-prefill QKV and residual/RMSNorm fusions")
...
def run_case(torch, cpp, engine, case, args, requests):
    ...
    if not args.fa3_only and not args.fa3_fusions and not args.prefill_fusions:
    ...
    if args.prefill_fusions:
        # Decode is deliberately identical across these arms. The accepted FA3
        # residual/QKV decode path remains enabled so this measures the prefill
        # changes on top of the current engine rather than an obsolete baseline.
        prefill_common = dict(
            model=engine.model, pool=pool, loop=None, **common,
            decode_attention_policy="fa3",
            max_capture_tokens=args.max_capture_tokens,
        ...
        adapters["piecewise_fa3_prefill_control"] = PiecewiseGraphModelAdapter(
            **prefill_common,
        )
        adapters["piecewise_fa3_prefill_qkv_epilogue"] = PiecewiseGraphModelAdapter(
            **prefill_common, enable_prefill_packed_qkv_rope_cache=True,
        )
        adapters["piecewise_fa3_prefill_residual_rmsnorm"] = PiecewiseGraphModelAdapter(
            **prefill_common, enable_prefill_residual_rmsnorm=True,
        )
        adapters["piecewise_fa3_prefill_both"] = PiecewiseGraphModelAdapter(
            **prefill_common, enable_prefill_packed_qkv_rope_cache=True,
            enable_prefill_residual_rmsnorm=True,
        )
    ...
    elif args.prefill_fusions:
        active_arms = PREFILL_FUSION_ARMS
    else:
    ...
    decision = ({"choice": "separate_run_not_paired"}
                if (args.splitk_only or args.fa3_only or args.fa3_compare
                    or args.fa3_fusions or args.prefill_fusions) else
    ...
    return {"status": "ok", "case_id": case["id"], "actual_work": actual_work,
            ...
            "fusion_features": {
                arm: {
                    ...
                    "prefill_packed_qkv_epilogue": bool(getattr(
                        adapter, "enable_prefill_packed_qkv_rope_cache", False)),
                    "prefill_residual_rmsnorm": bool(getattr(
                        adapter, "enable_prefill_residual_rmsnorm", False)),
                }
...
def main():
    ...
    if args.splitk_only:
        ...
        planned_arms = PREFILL_FUSION_ARMS
    else:
        planned_arms = ARMS + (("piecewise_fa3",) if args.include_fa3 else ())
    ...
    if args.check_setup:
        ...
        if any("fa3" in arm for arm in planned_arms):
